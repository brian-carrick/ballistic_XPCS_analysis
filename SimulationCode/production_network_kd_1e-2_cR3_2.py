...
initial_network = 'initial_network_kd_1e-2_cR3_2.txt'
...
netgen_flag = 0# if 0: read network from file, if 1: generate network using Gusev’s method
...
fbe=open('bond_exchange_stats','w')  # NEW FILE FOR BOND EXCHANGE STATS
...
fbe_detailed = open('bond_exchange_detailed.txt', 'w')
...
ioLAMMPS.writeLAMMPS(initial_network, mymin.xlo, mymin.xhi, mymin.ylo, mymin.yhi, mymin.zlo, 
...
for step in range(bond_exchange_steps):
    ...
    if((step+1) % wrt_step == 0): 
        filename = 'network_kd_1e-2_cR3_2_step_%d.txt' %(step+1)
...
filename = 'final_network_kd_1e-2_cR3_2.txt'
